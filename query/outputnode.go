...
func (enc *encoder) AddMapChild(fj, val fastJsonNode) {
	...
	if childNode == nil {
		...
		// For non-list predicates, replace the existing child entirely
		// instead of merging their children. This prevents duplicate fields
		// when mutations delete and then set the same predicate in one transaction.
		if prevChild == nil {
			// childNode is the first child, replace it
			val.next = childNode.next
			fj.child = val
		} else {
			// childNode is not the first child, update the previous node's next pointer
			val.next = childNode.next
			prevChild.next = val
		}
	}
