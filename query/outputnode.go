...
func (enc *encoder) AddMapChild(fj, val fastJsonNode) {
	...
	if childNode == nil {
		...
		// For non-list predicates, replace the existing child instead of merging.
		// This handles cases where mutations in the same transaction (delete + set)
		// would otherwise create duplicate fields in JSON output.
		if prevChild == nil {
			// childNode is the first child
			fj.child = childNode.next
		} else {
			// childNode is not the first child
