...
func (sg *SubGraph) preTraverse(enc *encoder, uid uint64, dst fastJsonNode) error {
	...
	for _, pc := range sg.Children {
		...
		case idx < len(pc.uidMatrix) && len(pc.uidMatrix[idx].Uids) > 0:
			...
			// For non-list predicates with multiple UIDs (can happen when multiple mutations
			// in a transaction modify the same edge), only process the last UID to avoid
			// duplicate fields in JSON output.
			startIdx := 0
